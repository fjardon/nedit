static const char CVSID[] = "$Id: highlightData.c,v 1.52.2.1 2003/07/11 10:39:47 edg Exp $";
/*******************************************************************************
*									       *
...
/* The VAX C compiler cannot compile this definition */
    ...
    "NEdit Macro:2:0{\n\
        README:\"NEdit Macro syntax highlighting patterns, version 2.3, maintainer Thorsten Haude, nedit at thorstenhau.de\":::Flag::D\n\
        Comment:\"#\":\"$\"::Comment::\n\
        Built-in Misc Vars:\"(?<!\\Y)\\$(?:active_pane|calltip_ID|column|cursor|display_width|empty_array|file_name|file_path|language_mode|line|locked|max_font_width|min_font_width|modified|n_display_lines|n_panes|rangeset_list|read_only|selection_(?:start|end|left|right)|server_name|text_length|top_line)>\":::Identifier::\n\
        ...
        Built-in Subrs:\"<(?:append_file|beep|calltip|clipboard_to_string|dialog|focus_window|get_character|get_pattern|get_range|get_selection|get_style|getenv|kill_calltip|length|list_dialog|max|min|rangeset_(?:create|destroy|add|subtract|invert|info|range|includes|set_color|set_mode)|read_file|replace_in_string|replace_range|replace_selection|replace_substring|search|search_string|select|select_rectangle|set_cursor_pos|set_language_mode|set_locked|shell_command|split|string_compare|string_dialog|string_to_clipboard|substring|t_print|tolower|toupper|valid_number|write_file)>\":::Subroutine::\n\
